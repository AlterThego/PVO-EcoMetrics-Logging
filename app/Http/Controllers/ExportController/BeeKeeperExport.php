...
class BeeKeeperExport extends Controller
{
    public function generateExcel()
    {
        ...
        $sheet->setCellValue('A5', 'Bee Keeping');
        ...
        $data = BeeKeeper::orderBy('year', 'desc')->get();
        ...
        $sheet->setCellValue('D7', 'Colonies');
        $sheet->setCellValue('E7', 'Bee Keepers');
        ...
        foreach ($data as $item) {
            ...
            $sheet->setCellValue('D' . $row, $item->colonies);
            $sheet->setCellValue('E' . $row, $item->bee_keepers);
        ...
        $tempFilePath = tempnam(sys_get_temp_dir(), 'bee_keepers_');
        // Save the Excel file to the temporary path
        $writer = new Xlsx($spreadsheet);
        $writer->save($tempFilePath);

        // Download the Excel file
        $fileName = 'benguet_bee_keeping.xlsx';
