...
class FarmSuppliesExport extends Controller
{
    public function generateExcel()
    {
        ...
        $sheet->setCellValue('A5', 'Farm Supplies');
        ...
        $data = FarmSupply::orderBy('year_established', 'desc')->get();


        // Add headers to the spreadsheet
        $sheet->setCellValue('A7', 'Municipality');
        $sheet->setCellValue('B7', 'Municipality');
        $sheet->setCellValue('C7', 'Establishement Name');
        $sheet->setCellValue('D7', 'Year Established');
        $sheet->setCellValue('E7', 'Year Closed');
        ...
        foreach ($data as $item) {
            ...
            $sheet->setCellValue('C' . $row, $item->establishment_name);
            $sheet->setCellValue('D' . $row, $item->year_established);
            $sheet->setCellValue('E' . $row, $item->year_closed);
        ...
        $tempFilePath = tempnam(sys_get_temp_dir(), 'farm_supplies_');
        // Save the Excel file to the temporary path
        $writer = new Xlsx($spreadsheet);
        $writer->save($tempFilePath);

        // Download the Excel file
        $fileName = 'benguet_farm_supplies.xlsx';
