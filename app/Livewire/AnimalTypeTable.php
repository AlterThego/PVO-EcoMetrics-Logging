...
final class AnimalTypeTable extends PowerGridComponent
{
    ...
    public function actions(AnimalType $row): array
    {
        $actions = [];
        if (!$row->trashed()) {
            $actions[] = Button::add('update-modal.animal-type-update')
                ...
                ->openModal('update-modal.animal-type-update', ['animalTypeUpdateId' => $row->id]);
            ...
            $actions[] = Button::add('restore-row')
                ...
                ->openModal('restore-row', ['animalTypeId' => $row->id]);
        }
