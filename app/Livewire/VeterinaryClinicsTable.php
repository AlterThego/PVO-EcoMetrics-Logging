...
final class VeterinaryClinicsTable extends PowerGridComponent
{
    ...
    public function actions(VeterinaryClinics $row): array
    {
        $actions = [];
        if (!$row->trashed()) {
            $actions[] = Button::add('update-modal.veterinary-clinics-update')
                ...
                ->openModal('update-modal.veterinary-clinics-update', ['veterinaryClinicsUpdateId' => $row->id]);
            ...
            $actions[] = Button::add('restore-row')
                ...
                ->openModal('restore-row', ['veterinaryClinicsId' => $row->id]);
        }
        return $actions;
    }
