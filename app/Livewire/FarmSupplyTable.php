...
final class FarmSupplyTable extends PowerGridComponent
{
    ...
    public function actions(FarmSupply $row): array
    {
        $actions = [];
        if (!$row->trashed()) {
            $actions[] = Button::add('update-modal.farm-supply-update')
                ...
                ->openModal('update-modal.farm-supply-update', ['farmSupplyUpdateId' => $row->id]);
            ...
            $actions[] = Button::add('delete-row')
                ...
                ->openModal('delete-row', ['farmSuppliesId' => $row->id]);
        } else {
            $actions[] = Button::add('restore-row')
                ...
                ->openModal('restore-row', ['farmSuppliesId' => $row->id]);
        }
