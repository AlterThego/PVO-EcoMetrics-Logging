...
final class DiseaseTable extends PowerGridComponent
{
    ...
    public function actions(Disease $row): array
    {
        $actions = [];
        if (!$row->trashed()) {
            $actions[] = Button::add('update-modal.disease-update')
                ...
                ->openModal('update-modal.disease-update', ['diseaseUpdateId' => $row->id]);
            ...
            $actions[] = Button::add('delete-row')
                ...
                ->openModal('delete-row', ['diseaseId' => $row->id]);
        } else {
            $actions[] = Button::add('restore-row')
                ...
                ->openModal('restore-row', ['diseaseId' => $row->id]);
        }
