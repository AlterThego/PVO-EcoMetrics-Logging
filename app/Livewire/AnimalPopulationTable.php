...
final class AnimalPopulationTable extends PowerGridComponent
{
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make('Volume (LW in MT)', 'volume')
                ->sortable()
                // ->editOnClick(true)
                ->searchable(),
    ...
    public function actions(AnimalPopulation $row): array
    {
        $actions = [];
        if (!$row->trashed()) {
            $actions[] = Button::add('update-modal.animal-population-update')
                ...
                ->openModal('update-modal.animal-population-update', ['animalPopulationUpdateId' => $row->id]);
            ...
            $actions[] = Button::add('restore-row')
                ...
                ->openModal('restore-row', ['animalPopulationId' => $row->id]);
        }
        return $actions;
    }
