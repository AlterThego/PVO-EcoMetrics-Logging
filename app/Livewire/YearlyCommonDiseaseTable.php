...
final class YearlyCommonDiseaseTable extends PowerGridComponent
{
    ...
    public function actions(YearlyCommonDisease $row): array
    {
        $actions = [];
        if (!$row->trashed()) {
            $actions[] = Button::add('update-modal.yearly-common-disease-update')
                ...
                ->openModal('update-modal.yearly-common-disease-update', ['yearlyCommonDiseaseUpdateId' => $row->id]);
            ...
            $actions[] = Button::add('restore-row')
                ...
                ->openModal('restore-row', ['yearlyCommonDiseaseId' => $row->id]);
        }
        return $actions;
    }
    /*
