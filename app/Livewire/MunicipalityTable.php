...
final class MunicipalityTable extends PowerGridComponent
{
    ...
    public function actions(Municipality $row): array
    {
        $actions = [];

        // Check if the municipality is not soft-deleted
        if (!$row->trashed()) {
            // Add edit button
            $actions[] = Button::add('update-modal.municipality-update')
                ...
                ->openModal('update-modal.municipality-update', ['municipalityUpdateId' => $row->id]);
            // Add delete button
            $actions[] = Button::add('delete-row')
                ...
                ->openModal('delete-row', ['municipalityId' => $row->id]);
        } else {
            $actions[] = Button::add('restore-row')
                ...
                ->openModal('restore-row', ['municipalityId' => $row->id]);
        }
        return $actions;
    }
