...
final class FishProductionTable extends PowerGridComponent
{
    // use WithExport;
    ...
    public function actions(FishProduction $row): array
    {
        $actions = [];

        if (!$row->trashed()) {
            $actions[] = Button::add('update-modal.fish-production-update')
                ...
                ->openModal('update-modal.fish-production-update', ['fishProductionUpdateId' => $row->id]);
            ...
            $actions[] = Button::add('delete-row')
                ...
                ->openModal('delete-row', ['fishProductionId' => $row->id]);
        } else {
            $actions[] = Button::add('restore-row')
                ...
                ->openModal('restore-row', ['fishProductionId' => $row->id]);
        }
