...
final class PopulationTable extends PowerGridComponent
{
    ...
    public function datasource(): Builder
    {
        return Population::query()
            ->join('municipalities', 'populations.municipality_id', '=', 'municipalities.id')
            ->select(
                'populations.*',
    ...
    public function addColumns(): PowerGridColumns
    {
        return PowerGrid::columns()
            ...
            ->addColumn('census_year')
            ->addColumn('population_count')
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make('Population count', 'population_count')
                ->sortable()
                ->searchable(),
    ...
    public function actions(Population $row): array
    {
        ...
        if (!$row->trashed()) {
            $actions[] = Button::add('update-modal.population-update')
                ...
                ->openModal('update-modal.population-update', ['populationUpdateId' => $row->id]);
            ...
            $actions[] = Button::add('delete-row')
                ...
                ->openModal('delete-row', ['populationId' => $row->id]);
        } else {
            $actions[] = Button::add('restore-row')
                ...
                ->openModal('restore-row', ['populationId' => $row->id]);
        }
        return $actions;
    }
