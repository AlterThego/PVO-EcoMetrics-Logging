...
final class FishSanctuaryTable extends PowerGridComponent
{
    ...
    public function actions(FishSanctuary $row): array
    {
        ...
        if (!$row->trashed()) {
            $actions[] = Button::add('update-modal.fish-sanctuary-update')
                ...
                ->openModal('update-modal.fish-sanctuary-update', ['fishSanctuaryUpdateId' => $row->id]);
            ...
            $actions[] = Button::add('restore-row')
                ...
                ->openModal('restore-row', ['fishSanctuariesId' => $row->id]);
        }
        return $actions;
    }
