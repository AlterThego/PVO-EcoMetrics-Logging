...
final class BeeKeeperTable extends PowerGridComponent
{
    ...
    public function actions(BeeKeeper $row): array
    {
        $actions = [];
        if (!$row->trashed()) {
            $actions[] = Button::add('update-modal.bee-keeper-update')
                ...
                ->openModal('update-modal.bee-keeper-update', ['beeKeepersUpdateId' => $row->id]);
            ...
            $actions[] = Button::add('restore-row')
                ...
                ->openModal('restore-row', ['beeKeeperId' => $row->id]);
        }
        return $actions;
    }
    /*
