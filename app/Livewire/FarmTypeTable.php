...
final class FarmTypeTable extends PowerGridComponent
{
    ...
    public function actions(FarmType $row): array
    {
        $actions = [];
        if (!$row->trashed()) {
            $actions[] = Button::add('update-modal.disease-update')
                ...
                ->openModal('update-modal.farm-type-update', ['farmTypeUpdateId' => $row->id]);
            ...
            $actions[] = Button::add('delete-row')
                ...
                ->openModal('delete-row', ['farmTypeId' => $row->id]);
        } else {
            $actions[] = Button::add('restore-row')
                ...
                ->openModal('restore-row', ['farmTypeId' => $row->id]);
        }
